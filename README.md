...
This workshop is designed to be run leveraging VSCode DevContainers. 

### 1. Local Desktop

After cloning this repository re-open it in a dev container. This should begin the `yarn install` process, ensure it is running and wait for it to finish. If it does not run automatically you can open a terminal within vscode and run the command yourself.
...
2. Go to [codespaces](https://github.com/features/codespaces)
3. Click `Get started for free`
![code-spaces-1](./screenshots/code-spaces-1.png)
4. Click `New Codespace`
![code-spaces-2](./screenshots/code-spaces-2.png)
5. Select the following options
  - Repository: `The forked backstage-demo repo`
  - Branch: `main`
  - Dev container configuration: `Node.js`
  - Region: `US East`
  - Machine type: `4-core`
...
#### Codespaces Only
From here on the workshop should follow the same instructions with one exception. Any reference to `localhost` will need to replaced by the custom url generated by codespaces. VSCode should help you out with this by asking if you want to open any forwarded ports in your browser. We will also have to make the following changes to the `app-config.yaml` file.

`app-config.yaml`
1. Update app baseUrl to codespaces generated.
![code-spaces-config-1](./screenshots/cs-config-1.png)
1. Update backend baseUrl to codespaces generated.
![code-spaces-config-2](./screenshots/cs-config-2.png)
1. Update cros to allow codespaces generated.
![code-spaces-config-3](./screenshots/cs-config-3.png)
1. Restart backstage `ctrl+c` followed by `yarn dev`
1. Wait for it to come up again and then make the backend url publicly accessible.
![code-spaces-config-4](./screenshots/cs-config-4.png)
1. Refresh backstage portal in your browser

NOTE: Every time backstage is restarted you may need to make your backend port `7007` public again.

### Explore Backstage
Backstage will open on http://localhost:3000.

You will need to login as a guest and you may receive the error below. You can safely ignore it.

![login-error](./screenshots/login-error.png)

Next you can explore the different components of backstage.
